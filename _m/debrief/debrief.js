$(function() {
    ...
    */
	...
	$("#closeP").on("click", function(e) {e.preventDefault()
        ...
        $('#fx1').trigger("stop").attr("src", "").attr("src", "/data/audiosys/Device-start-up-sound-effect.mp3")
        $('#fx2').trigger("stop").attr("src", "").attr("src", "/data/audiosys/Mario_Jumping-Mike_Koenig-989896458.mp3")
        $('#aask').trigger("stop").attr("src", "")
        $('#aansw').trigger("stop").attr("src", "")
...
startMatchMovie
	playTrig
		loadSeq=function(s, status=null, info=null)
			playSeq(s, status, info)
                playTrg or final
