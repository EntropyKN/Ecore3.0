...
/************** avatars ************* */
$QAF=sql_query	("SELECT id, name FROM avatars where id!=1000 AND invisble=0 AND (propertyUid is null OR propertyUid=".$_SESSION["uid"].") order by id");
while (	$avatar=sql_assoc($QAF)	){
    $f=$avatar["id"];
//for ($f = 0; $f < 11; $f++) {
	$C1.=".avatar_".$f ."{background: url('/data/avatar_prev/1/".$f ."_10Kx1098_wait.png') left center;}
";
	
	if ($f) $C1.=".avatar_".$f ."_talk {background: url('/data/avatar_prev/1/".$f ."_10Kx1098_talk.png?a2') left center;}
";

	if ($f)   $C2.="url('/data/avatar_prev/1/".$f ."_10Kx1098_talk.png?b2') ";
